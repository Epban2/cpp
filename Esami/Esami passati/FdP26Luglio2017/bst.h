...
class BST 
  ...
  void rand_insertR(link& h, Item x)
   ...
   void crea_albero_somme_nodiR(link& h, BST<Item, Key>& b)
   {
	   if (h == NULL)  return;

	   //eseguo una visita dell'albero inversa rispetto alla visita In-order. In questo modo
	   //visito gli elementi in ordine di chiave decrescente
	   crea_albero_somme_nodiR(h->r, b);

	   //per ogni nodo dell'albero di partenza creo un nuovo nodo e lo inserisco nel secondo albero con prezzo pari alla chiave del nodo dell'albero di partenza a cui sommo tutte le chiavi più grandi
	   somma_nodi += h->item.key();
	   Item to_insert(h->item.gettitolo(), h->item.getcodice(), h->item.geteditore(), somma_nodi);
	   b.insert(to_insert);

	   crea_albero_somme_nodiR(h->l, b);
   }
	...
	public:
	  ...
	  void balance()
		{ balanceR(head);}
	  void crea_albero_somme_nodi(BST<Item, Key>& b)
	  {
		  somma_nodi = 0;
		  crea_albero_somme_nodiR(head,b);
	  }
  };
