...
class BST 
  ...
  void rand_insertR(link& h, Item x)
   ...
   int somma_foglie_sinistreR(link& h)
   {
	   int res = 0;
	   if (h != NULL)
	   {
		   if (is_leaf(h->l))
		   {
			   ...
			   res += somma_foglie_sinistreR(h->l);
		   res += somma_foglie_sinistreR(h->r);
	   }
	   return res;
   }
	...
	public:
	  ...
	  void balance()
		{ balanceR(head);}
	  int somma_foglie_sinistre()
	  {
		  return somma_foglie_sinistreR(head);
	  }
