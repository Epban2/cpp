...
private:
	...
	*/
	...
	void decrementaR(link& h) {
		if (h == 0) return;

		decrementaR(h->l);
		h->item.setVelocita(h->item.getvelocita() - 0.5);
		decrementaR(h->r);
	}

	void deleteCompagniaR(link& h, string compagnia) {
		if (h == 0) return;


		deleteCompagniaR(h->l, compagnia);
		deleteCompagniaR(h->r, compagnia);

		if (h->item.key() == compagnia)
		{
...
public:
	...
	void decrementa() { decrementaR(head); }

	void deleteCompagnia(string compagnia) { return deleteCompagniaR(head, compagnia); }
