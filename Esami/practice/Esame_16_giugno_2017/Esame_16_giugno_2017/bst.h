...
private:
	...
	/*
	Scrivere una procedura ricorsiva per contare quanti nodi foglia dell’albero hanno una quantità di
	prodotto minore o uguale a 25 e stampare i loro nomi
	*/
	int higherQuantityR(link h) {
		if (h == 0) //caso default, nodo nullo
			return 0;

		if (h->l == 0 && h->r == 0 && h->item.getQuantita() <= 25) {
			cout << h->item.key() << " ";
			return 1;
		}
		return higherQuantityR(h->l) + higherQuantityR(h->r);
...
public:
	...
	int higherQuantity() { return higherQuantityR(head); }
};
