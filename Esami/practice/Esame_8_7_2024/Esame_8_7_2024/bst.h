...
private:
	...
	//Stampa con visita in post ordine
	...
	*/
	...
	/*modificare l’elemento con chiave massima aggiungendo "***" in testa al <nome>.
	Non è consentito l’uso del metodo select.*/
	void editMaxKeyR(link& h) {
		//bisogna andare tutto a destra finché il figlio non è null -> a quel punto padre
		link father = h;
		link right_son = h->r;
		while (right_son != 0)
		{
			father = right_son;
			right_son = father->r;
		} //quanto il figlio è null siamo all'estremo
		father->item.setNome("***" + father->item.getNome());
	}
	
	/*modificare l’elemento con chiave mediana aggiungendo "*" in testa al <nome>.
	Non è consentito l’uso del metodo select.*/
	void editMiddleValueR() {
		//l'elemento con chiave mediana in un albero bilanciato è la radice!
...
public:
	...
	void editMaxKey() { editMaxKeyR(head); }
	void editMiddleValue() { editMiddleValueR(); }
