...
public:
	...
	/*
	trasforma il grafo suo grafo opposto.
	L’opposto di un grafo è il grafo con il verso di tutti gli archi invertito.
	*/
	void reverse() {
		List<Edge>** nuovaLista = (List<Edge>**) new List<Edge>*[numVertex];
		for (int i = 0; i < numVertex; i++)
			nuovaLista[i] = new LList<Edge>();

		for (int i = 0; i < numVertex; i++)
		{
			for (int w = first(i); w < numVertex; w = next(i, w)) {
				//tutti i w, sono i nodi raggiungibili da i (i->w), dobbiamo quindi invertirli
				int peso = weight(i, w);
				Edge arcoInverso(i, peso);
				nuovaLista[w]->insert(arcoInverso);
			}
		}

		//ripulisco il grafo e lo riempio con gli archi inversi
		for (int i = 0; i < numVertex; i++) {
			delete vertex[i];
			vertex[i] = nuovaLista[i];
