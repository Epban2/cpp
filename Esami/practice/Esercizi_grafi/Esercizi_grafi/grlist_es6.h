...
public:
	...
	/*
	grafo complementare è il grafo ottenuto eliminando tutti gli
	...
	void complement() {
		...
		for (int i = 0; i < numVertex; i++) {
			for (int j = 0; j < numVertex; j++) {
				if (!isEdge(i, j)) {
					Edge e(j, 1); //costo momentaneo 1, capire come gestire i costi
					listaComplementare[i]->insert(e);
				}
			}
		}
		for (int i = 0; i < numVertex; i++) {
			...
			vertex[i] = listaComplementare[i];
		}
		delete[] listaComplementare;
	}
};
