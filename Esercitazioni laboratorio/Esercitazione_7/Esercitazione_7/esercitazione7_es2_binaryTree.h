/*
Esercizio 2: Scrivere una funzione membro pubblica “equal_tree_structure” della classe BinaryTree per
determinare se un albero ha la stessa struttura di un albero il cui puntatore al nodo radice viene passato
come argomento(due alberi A e B hanno la stessa struttura se sono entrambi vuoti, oppure se sono
entrambi non vuoti e i sottoalberi destri e sinistri delle radici hanno rispettivamente la stessa struttura).
...
Il metodo pubblico deve chiamare un metodo privato ricorsivo che accetta come parametro i puntatori
ai due alberi
...
public:
	...
	//due alberi A e B hanno la stessa struttura se sono entrambi vuoti, oppure se sono
	//entrambi non vuoti e i sottoalberi destri e sinistri delle radici hanno rispettivamente la stessa struttura).
	int equal_tree_structure(Node<T>* theRootB) {
		return equal_structure(root, theRootB);
	}
	//significa che se es. il figlio destro di un nodo dell'albero A e' vuoto, allora il figlio destro dello stesso nodo dell'albero B dovra' essere vuoto, altrimenti non sono identici
