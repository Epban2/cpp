...
//versione pi� efficiente che memorizza risultati calcolati in precedenza
...
// funzione per determinare se un numero � pari
int is_even(unsigned int n)
{
	if (n == 0) return 1; //Il numero è pari quando il precedente è dispari
	 else return(is_odd(n-1));
}

// funzione per determinare se un numero � dispari
...
// funzione per calcolare il massimo in un array
    Item max(Item a[], int l, int r)
      {
        if (l == r) return a[l]; //Se i due estremi r==l allora l'array è una singola cella e la ritorno
        int m = (l+r)/2; 
        Item u = max(a, l, m);
        Item v = max(a, m+1, r);
